...
public:
  ...
  bool init()
  {
    ...
    if (PGN.listen(portSteer_8888))
    {
      ...
      PGN.onPacket([&](AsyncUDPPacket packet)
                   { checkForPGNs(packet); }); // all the brackets and ending ; are necessary!
    }
  ...
  {
    if (packet.remotePort() != 9999 || packet.length() < 5)
      return; // make sure from AgIO
  ...
  {
    Serial.print("\r\n\n0x");
    Serial.print(_pgnNum, HEX);
    Serial.print(" (");
    Serial.print(_pgnNum);
    Serial.print(") - ");
  ...
  void SendUdpFreeForm(uint8_t _type, char _msg[], uint8_t _len, char _seconds, IPAddress dip, uint16_t dport)
  {
    ...
    {
      Serial.print("\r\n");
      Serial.print(ForTheWire[6], DEC);
      Serial.print("s timed ");
    }
    else
      Serial.print("\r\nOK ");

    Serial.print(" pop-up msg sent to AOG: ");
    Serial.print("\"");
