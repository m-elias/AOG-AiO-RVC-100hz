...
private:
    uint8_t all_Ch_Fast_Write_Output_and_Pd_only()
    { // write output data only to all channels
        wire_->beginTransmission(addr_);
        for (uint8_t i = 0; i < 4; ++i)
        {
    ...
    uint8_t single_Ch_Write_to_Reg(uint8_t ch)
    { // write all data to a single channel
        wire_->beginTransmission(addr_);
        // for (uint8_t i = 0; i < 4; ++i){
    ...
    uint8_t all_Ch_Write_to_Reg_and_Ee()
    { // write all EE data to register and eeprom for all channels
        wire_->beginTransmission(addr_);
    ...
    uint8_t single_Ch_Write_to_Reg_and_Ee(uint8_t ch)
    { // write all EE data to register and eeprom for a single channel
        wire_->beginTransmission(addr_);
