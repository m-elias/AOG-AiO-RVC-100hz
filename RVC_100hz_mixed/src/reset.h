...
public:
  ...
  bool update()
  {
    if (digitalRead(btnIO) == LOW)
    {
      ...
      if (millis() - btnPressTime >= longPressPeriod)
      { // btn held for more than longPressPeriod (10s default)
        while (digitalRead(btnIO) == LOW)
        { // flash LED while btn is held to indicate factory/firmware reset is triggered
          if (digitalRead(ledIO) == 1)
            delay(40);
          else
            delay(10);
          digitalWrite(ledIO, !digitalRead(ledIO));
      ...
      { // short btn press, do reboot instead of settings reset
        reboot();
      }
    }
    return false;
