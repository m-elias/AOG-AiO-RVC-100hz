...
public:
  ...
  void set(uint8_t _id, uint8_t _stage, bool _debug = false)
  {
    ...
    { // any other value, set OFF (stage: 0)
      _stage = 0;
      data[_id].redValue = 0;
      data[_id].greenValue = 0;
    }
  ...
  void updateLoop()
  {
    ...
    if (updateTimer > updatePeriod)
    {
      ...
      if (!blinkNextLoop)
      { // remove "if" to additionally flash blue during ON stage of blink cycle
      ...
      if (blueFlashState && millis() > blueFlashStartTime + blueFlashPeriod)
      {
        ...
        for (byte i = 0; i < NUM_LEDS; i++)
        {
          if (data[i].blueFlash)
          {
            ...
            if ((data[i].stage == STAGE2_RED_BLINK || data[i].stage == STAGE3_GREEN_BLINK))
            { // && !blinkNextLoop) {
              WS2811.setPixelColor(i, 0, 0, 0);
            }
            else
