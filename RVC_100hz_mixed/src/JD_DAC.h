...
*/
	...
	int16_t steerOutput(int16_t _tractorPWM)
	{ // pwmDrive +-(minPWM - Max Limit)
	...
	{
		if (isInit)
			return steeringWheelSensor[0]; // stored ch 0 reading
		else
			return 13600; // center value
	}
	...
	bool updateAdsReadingCh0()
	{
		...
		if (adsIndex == 0)
		{
			...
			if (steeringWheelSensor[0] > 60000)
				steeringWheelSensor[0] = 0;
	...
	void printSWSdata()
	{
		...
		if (currentPrintTime > lastPrintTime + 1000)
		{
			...
			for (byte n = 0; n < NUM_SWS; n++)
			{
				...
				if (steeringWheelSensor[n] < 10000)
					debugPrint(" ");
				if (steeringWheelSensor[n] < 1000)
					debugPrint(" ");
				if (steeringWheelSensor[n] < 100)
					debugPrint(" ");
				if (steeringWheelSensor[n] < 10)
					debugPrint(" ");
				debugPrint(steeringWheelSensor[n]);
			}
			lastPrintTime += 1000;
			if (currentPrintTime > lastPrintTime)
				lastPrintTime = currentPrintTime + 1000;
		}
