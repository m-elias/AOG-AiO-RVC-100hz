...
public:
  ...
  void printStatsReport(char *_name)
  {
    ...
    if (isActive)
    {
      if (highNumShort > highNumLong)
        highNumLong = highNumShort;
      if (lowNumShort < lowNumLong)
        lowNumLong = lowNumShort;
...
public:
  ...
  {
    if (isActive)
      accumulatedTime += _time - periodStartTime;
    isActive = false;
  }
  ...
  {
    if (isActive)
      timeOut(); // not sure this should be here. How to deal with an active usage timer when report is called?
    uint32_t t = millis();
  ...
  void update()
  {
    if (speedTimeoutTimer < 1000)
    {
      if (pulseUpdateTimer > 200) // 100 (10hz) seems to cause tone lock ups occasionally
      {
        ...
        if (speedPulse > 2.0)
        { // less then 2 hz (0.055 km/hr) doesn't work well with tone()
        ...
        if (ledPin > -1)
        {
          ...
          if (speedPulse > 2.000)
          { // less then 2 hz (0.55 km/hr) doesn't work well with tone()
    ...
    { // if speedTimeoutTimer hasn't updated in 1000 ms, turn off speed pulse
      noTone(outputPin);
      if (ledPin > -1)
        noTone(ledPin);
    }
